...
func TaskLock(lockName, clientId string) {
	...
	_, _ = Lk.Lock(lockName, clientId)

	log.Printf("%s %s %d, 执行加锁后的业务...", lockName, clientId, time.Since(t))
}

func TaskUnLock(lockName, clientId string) {
	log.Printf("%s %s,释放锁...", lockName, clientId)
	t := time.Now()
	_, _ = Lk.UnLock(lockName, clientId)

	log.Printf("%s %s %d,执行释放锁后的业务...", lockName, clientId, time.Since(t))
}
